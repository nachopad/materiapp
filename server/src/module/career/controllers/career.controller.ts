...
export class CareerController {
  ...
  async create(
    @Body(CollegeValidatePipe) createCareerDTO: CreateCareerDto,
  ): Promise<CareerResponseDto> {
  ...
  async updateCareer(
    @Param('id', new IdValidationPipe()) id: string,
    @Body(CollegeValidatePipe, SubjectValidatePipe)
    updateCareerDTO: UpdateCareerDto,
  ): Promise<CareerResponseDto> {
    const updateCareer = await this.careerService.updateCareer(
      id,
      updateCareerDTO,
    );
  ...
  async addSubjectToCareer(
    @Param('id', new IdValidationPipe()) id: string,
    @Body() subjectEmbedded: CreateSubjectEmbeddedDto,
  ): Promise<CareerResponseDto> {
    const careerWithSubjectsEmbedded =
      await this.careerService.addSubjectEmbedded(id, subjectEmbedded);
