...
| Materiapp-specific | `materiapp-{component}` | `materiapp-server`, `materiapp-client`, `materiapp-sdk-check` |
| Testing skill | `materiapp-test-{component}` | `materiapp-test-sdk`, `materiapp-test-api` |
...
## Decision: Materiapp-Specific vs Generic

```

Patterns apply to ANY project? → Generic skill (e.g., pytest, typescript)
Patterns are Materiapp-specific? → materiapp-{name} skill
Generic skill needs Materiapp info? → Add references/ pointing to Materiapp docs
...
| `license` | Yes | Always `Apache-2.0` for Materiapp |
| `metadata.author` | Yes | `materiapp` |
